...
```math
DQ_{Score}=0.40S_{bcd}+0.35S_{complete}+0.25S_{consistent}
```
...
1. Introduces BCD arithmetic error detection as a lightweight hardware-level validation mechanism for AIoT systems.

2. Proposes a composite Data Quality Score that dynamically controls safe-failure decision behavior.

3. Demonstrates that Agentic RAG combined with hardware validation significantly improves:
...
# 18. Final Contribution Statement
